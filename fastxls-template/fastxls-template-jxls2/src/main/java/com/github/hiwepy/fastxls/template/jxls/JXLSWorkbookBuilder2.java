...
/**
 */
public class JXLSWorkbookBuilder2<T extends CellModel> extends WorkbookBuilder2<T, Void> {
	...
	public Void build() {
		try {
			...
			if( this.getColumnList() != null) {
				context.putVar("columnList", this.getColumnList());
			}
			else if(this.getRowList() == null) {
				context.putVar("rowList", this.getRowList());
			}
			else if(this.getRowSet() == null) {
				context.putVar("rowSet", this.getRowSet());
			}
