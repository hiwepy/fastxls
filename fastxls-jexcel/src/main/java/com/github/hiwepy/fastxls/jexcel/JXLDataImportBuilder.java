...
public class JXLDataImportBuilder<T extends CellModel,M extends RowMap> 
	extends DataImportBuilder<Workbook, Sheet, Cell[], Cell, T, M> {
		
	private BiFunction<RowWrapper<Cell[]> , Integer, RowMapper<Cell[], RowMap>> rowmapMapper;
	private BiFunction<RowMapper<Cell[], RowMap>, Integer, Set<ConstraintViolation<RowMap>>> rowmapValidation;
	private BiFunction<RowWrapper<Cell[]>, Integer, RowMapper<Cell[], M>> rowDtoMapper;
	private BiFunction<RowMapper<Cell[], M>, Integer, Set<ConstraintViolation<M>>> rowDtoValidation;
	private BiFunction<RowWrapper<Cell[]>, Integer, Set<ConstraintViolation<M>>> rowValidation;
	
	public JXLDataImportBuilder() {
		this(new JXLWorkbookReader());
	}

	public JXLDataImportBuilder(JXLWorkbookReader workbookReader) {
		super(workbookReader);
	}
	
	public JXLDataImportBuilder(BiFunction<ColumnWrapper<Cell>, String, String> transformer) {
		this(new JXLWorkbookReader(), transformer);
	}
	
	public JXLDataImportBuilder(JXLWorkbookReader workbookReader, BiFunction<ColumnWrapper<Cell>, String, String> transformer) {
		super(workbookReader);
	...
	public ConstraintViolationResult<M> input(File file,
			...
			BiConsumer<RowWrapper<Cell[]>, Integer> consumer) throws Exception {
	...
	public ConstraintViolationResult<M> input(Sheet[] sheets,
		...
		// 计算总行数
		BigDecimal count = BigDecimal.ONE.max(BigDecimal.valueOf(Stream.of(sheets).mapToInt(sheet -> sheet.getRows()).sum()));
		int index = 0;
		// 是否使用多线程进行构建
		if(this.isThreadPool()) {
			// 计算每个sheet需要多少个线程进行处理,并累加计算总共需要线程数
			int threadNum = Math.max(1, Stream.of(sheets).mapToInt(sheet -> ExecutorUtils.getThreadCount(sheet.getRows(), this.getMaxThreadTaskSize())).sum());
			// 固定大小的创建线程池
			...
			for (Sheet sheet : sheets) {
				...
				// 计算当前Sheet数据需要的线程数
				int threadNumOfSheet = ExecutorUtils.getThreadCount(sheet.getRows(), this.getMaxThreadTaskSize());
				// 打印日志
				LOG.info(Constants.THREAD_INFO, sheet.getName(), sheet.getRows(), this.getMaxThreadTaskSize() , threadNumOfSheet);
				// 起始位
				int offset = 0;
				// 循环构建线程
				for (int threadIndex = 0; threadIndex <= threadNumOfSheet; threadIndex++) {
					// 线程名称
					String taskName = String.format(Constants.THREAD_TASKNAME, sheet.getName(), this.getThreadNamePrefix(), threadIndex);
					// 向线程池增加一个线程任务
					futureTaskList.add(cachedThreadPool.submit(new DataImportTask< Workbook, Sheet, Cell[], Cell, M>( 
			...
			for (Sheet sheet : sheets) {
				...
				for (int rowNum = 0; rowNum <= sheet.getRows(); rowNum++) {
					...
					// 行对象包装
					RowWrapper<Cell[]> wrapper = new RowWrapper<Cell[]>(row, columns , rowNum , this.getFirstDataRowOfSheet());
					// 执行验证
	...
	public ConstraintViolationResult<RowMap> input2(File file,
			BiFunction<Sheet, Integer, List<Map<String, String>>> header,
			BiConsumer<RowMapper<Cell[], RowMap>, Integer> consumer) throws Exception {
		Sheet[] sheets = getWorkbookReader().getSheets(file);
		...
		if(this.isThreadPool()) {
			...
			for (Sheet sheet : sheets) {
				...
				for (int threadIndex = 0; threadIndex <= threadNumOfSheet; threadIndex++) {
					...
					String taskName = String.format(Constants.THREAD_TASKNAME, sheet.getName(), this.getThreadNamePrefix(), threadIndex);
					// 向线程池增加一个线程任务
					futureTaskList.add(cachedThreadPool.submit(new MappedDataImportTask< Workbook, Sheet, Cell[], Cell, RowMap>( 
							taskName, sheet, offset, this.getMaxThreadTaskSize(), this.getFirstDataRowOfSheet(), 
			...
			for (Sheet sheet : sheets) {
				...
				for (int rowNum = 0; rowNum <= sheet.getRows(); rowNum++) {
					...
					RowWrapper<Cell[]> wrapper = new RowWrapper<Cell[]>(row, columns ,rowNum, this.getFirstDataRowOfSheet());
					// 进行行数据转换
					RowMapper<Cell[], RowMap> mapper = getRowmapMapper().apply(wrapper, rowNum);
					// 执行验证
	...
	public ConstraintViolationResult<M> input3(File file,
			BiFunction<Sheet, Integer, List<Map<String, String>>> header,
			BiConsumer<RowMapper<Cell[], M>, Integer> consumer) throws Exception {
		Sheet[] sheets = getWorkbookReader().getSheets(file);
		...
		if(this.isThreadPool()) {
			...
			for (Sheet sheet : sheets) {
				...
				for (int threadIndex = 0; threadIndex <= threadNumOfSheet; threadIndex++) {
					...
					String taskName = String.format(Constants.THREAD_TASKNAME, sheet.getName(), this.getThreadNamePrefix(), threadIndex);
					// 向线程池增加一个线程任务
					futureTaskList.add(cachedThreadPool.submit(new MappedDataImportTask< Workbook, Sheet, Cell[], Cell, M>( 
							taskName, sheet, offset, this.getMaxThreadTaskSize(), this.getFirstDataRowOfSheet(), 
			...
			for (Sheet sheet : sheets) {
				...
				for (int rowNum = 0; rowNum <= sheet.getRows(); rowNum++) {
					...
					RowWrapper<Cell[]> wrapper = new RowWrapper<Cell[]>(row, columns ,rowNum, this.getFirstDataRowOfSheet());
					// 进行行数据转换
					RowMapper<Cell[], M> mapper = getRowDtoMapper().apply(wrapper, rowNum);
					// 执行验证
	...
	public ConstraintViolationResult<M> input(File file, 
			BiFunction<Sheet, Integer, List<Map<String, String>>> header,
			BiFunction<RowWrapper<Cell[]>, Integer, RowMapper<Cell[],M>> rowMapper,
			BiFunction<RowMapper<Cell[],M>, Integer, Set<ConstraintViolation<M>>> validation, 
			BiConsumer<RowMapper<Cell[],M>, Integer> consumer)
			throws Exception {
		Sheet[] sheets = getWorkbookReader().getSheets(file);
		...
		if(this.isThreadPool()) {
			...
			for (Sheet sheet : sheets) {
				...
				for (int threadIndex = 0; threadIndex <= threadNumOfSheet; threadIndex++) {
					...
					String taskName = String.format(Constants.THREAD_TASKNAME, sheet.getName(), this.getThreadNamePrefix(), threadIndex);
					// 向线程池增加一个线程任务
					futureTaskList.add(cachedThreadPool.submit(new MappedDataImportTask< Workbook, Sheet, Cell[], Cell, M>( taskName, sheet, offset, this.getMaxThreadTaskSize() , 
							this.getFirstDataRowOfSheet(), columns, this.isIgnoreException(), this.getWorkbookReader(), rowMapper, validation, consumer)));
					// 计算下一个开始位置
					offset += this.getMaxThreadTaskSize();
				}
				sheetNum += 1;
			...
			for (Sheet sheet : sheets) {
				...
				for (int rowNum = 0; rowNum <= sheet.getRows(); rowNum++) {
					...
					Cell[] row = sheet.getRow(rowNum);
					// 数据转换
					RowMapper<Cell[], M> mapper = rowMapper.apply(new RowWrapper<Cell[]>(row, columns, rowNum, this.getFirstDataRowOfSheet()), rowNum);
					// 判断是否表头行
					if(mapper.isTitle()) {
						continue;
					}
	...
	public BiFunction<RowWrapper<Cell[]>, Integer, RowMapper<Cell[], RowMap>> getRowmapMapper() {
		return rowmapMapper;
	}

	public void setRowmapMapper(BiFunction<RowWrapper<Cell[]>, Integer, RowMapper<Cell[], RowMap>> rowmapMapper) {
		this.rowmapMapper = rowmapMapper;
	}

	public BiFunction<RowMapper<Cell[], RowMap>, Integer, Set<ConstraintViolation<RowMap>>> getRowmapValidation() {
		return rowmapValidation;
	}

	public void setRowmapValidation(
			BiFunction<RowMapper<Cell[], RowMap>, Integer, Set<ConstraintViolation<RowMap>>> rowmapValidation) {
		this.rowmapValidation = rowmapValidation;
	}

	public BiFunction<RowWrapper<Cell[]>, Integer, RowMapper<Cell[], M>> getRowDtoMapper() {
		return rowDtoMapper;
	}

	public void setRowDtoMapper(BiFunction<RowWrapper<Cell[]>, Integer, RowMapper<Cell[], M>> rowDtoMapper) {
		this.rowDtoMapper = rowDtoMapper;
	}

	public BiFunction<RowMapper<Cell[], M>, Integer, Set<ConstraintViolation<M>>> getRowDtoValidation() {
		return rowDtoValidation;
	}

	public void setRowDtoValidation(BiFunction<RowMapper<Cell[], M>, Integer, Set<ConstraintViolation<M>>> rowDtoValidation) {
		this.rowDtoValidation = rowDtoValidation;
	}

	public BiFunction<RowWrapper<Cell[]>, Integer, Set<ConstraintViolation<M>>> getRowValidation() {
		return rowValidation;
	}

	public void setRowValidation(
			BiFunction<RowWrapper<Cell[]>, Integer, Set<ConstraintViolation<M>>> rowValidation) {
