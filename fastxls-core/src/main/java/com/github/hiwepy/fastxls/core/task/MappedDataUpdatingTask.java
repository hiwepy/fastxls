...
/**
 * Excel 数据更新线程（Row对象映射为Model对象，借助Validator对象进行数据行校验）
 ...
 * @param <M>
 */
public class MappedDataUpdatingTask<W, S, R, C, M extends RowMap> implements Callable<List<RowConstraintViolation<M>>> {
	...
	private final BiConsumer<RowMapper<R, M>, Integer> consumer;
	
	public MappedDataUpdatingTask(
		    ...
		    BiConsumer<RowMapper<R, M>, Integer> consumer
		   ){
	...
	public List<RowConstraintViolation<M>> call() throws Exception {
		...
		if(CollectionUtils.isNotEmpty(rows)){
			...
			for (int rowNum = this.getOffset(); rowNum < this.getOffset() + rows.size(); rowNum++) {
				...
				// 调用消费函数 
				getConsumer().accept(mapper, rowNum);
			}
			//记录状态日志
	...
	public BiConsumer<RowMapper<R, M>, Integer> getConsumer() {
		return consumer;
	}
