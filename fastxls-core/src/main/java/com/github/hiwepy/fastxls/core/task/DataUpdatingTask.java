 ...
 * @param <M>
 */
public class DataUpdatingTask<W, S, R, C, M extends RowMap> implements Callable<List<RowConstraintViolation<M>>> {
	...
	public List<RowConstraintViolation<M>> call() throws Exception {
		...
		if(CollectionUtils.isNotEmpty(rows)){
			...
			for (int rowNum = this.getOffset(); rowNum < this.getOffset() + rows.size(); rowNum++) {
				...
				// 调用消费函数 
				getConsumer().accept(new RowWrapper<R>(row, columns, rowNum, firstDataRowOfSheet), rowNum);
			}
			//记录状态日志
